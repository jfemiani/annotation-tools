...
class PolygonEditor(object):
    ...
    - x/y + lmb: Costrain motion to the x/y axis
    - n + lmb: Slide an edge keeping all lines parallel to their original direction
    - w: Split the active (or last) edge at a pount (at the cursor)
    ...
    def __init__(self, x, y=None,  ax=None,
        ...
        - *onupdate:* A callback invoked with the `x` and `y` arrays whenever 
          they are changed. 
        ...
        self.keymap = {
            ...
            'e': 'collapse-edge',  # Collapes the active (or last) edge to a point
    ...
    def push(self):
        ...
        # No more redos available after you make an edit
    ...
    def redo(self):
        """Restore state to a pount in history (moving forward)"""
    ...
    def fit_point_in_view(self, x=None, y=None):
        if x is None or y is None:
            x = np.mean([self.x[i] for i in self._active_verts])
            y = np.mean([self.y[i] for i in self._active_verts])
    ...
    def delete_vertex(self, i=None):
        """Delete a vertex (and select the new edge)"""
        if i is None:
            i = self._active_verts[0]
        
        del self.x[i]
        del self.y[i]
        self._active_verts = sorted([(i-1)%len(self), i])
        
    def move_active(self, dx, dy):
        if not self._active_verts:
            self._active_verts = list(range(len(self)))
        for vi in self._active_verts:
            self.x[vi] += dx
            self.y[vi] += dy
        self.push()
    ...
    def _on_key_press(self, event):
        ...
        if self.keymap.get(event.key) == 'split-edge':
            ...
            self._active_verts = list(range(len(self)+1))
    ...
    def select_next_edge_or_vertex(self, single=True, direction=+1):
        if len(self._active_verts) == 0:
            self._active_verts = [0]
        elif len(self._active_verts) == 1:
            self._active_verts.append((self._active_verts[-1]+direction)%len(self))
        elif len(self._active_verts) == 2:
            v0, v1 = self._active_verts
            if v0 == (v1+1) %len(self):
                v1, v0 = v0, v1
            if direction == +1:
                self._active_verts = [v1]
            else:
                self._active_verts = [v0]
            
        if single:
            self._active_verts = [self._active_verts[-1]]
        else: 
            self._active_verts = sorted(self._active_verts)
        av = self._active_verts[-1]
        ...
        if event.inaxes == self.ax:
            ...
            if self.state == 'picking' or (self.state == 'adding' and event.key == 'control'):
                if self.keymap.get(event.key) == 'select-all':
                    ...
                    # We are selecting a point or an adge
                    self._active_verts = []
                    i, d = self.closest_point((event.x, event.y))
                    if i >= 0:
                        self._active_verts = [i]
                    else:
                        i, d  = self.closest_edge((event.x, event.y))
                        if i >= 0:
                            self._active_verts = [i, (i+1)%len(self)]
    ...
    def _drag_active(self, x, y, key):
        # While we are not complete, we are adding to the end by defaule
        if not self.completed and key != 'control':
            self._active_verts = [len(self)]  
        
        if self._down_event is None:
            ...
            if key == 'n' and len(self._active_verts) == 2:          
                #Two points on the edge, and two points just beyond
                i1 = self._active_verts[0]
                i2 = self._active_verts[-1]
                i0 = (i1-1)%len(self)
                i3 = i2 + 1
                ...
                # New pointe are at the indersections of the liens
                q1 = np.cross(l0, l1)
                q1 /= q1[2]
                ...
                for i in self._active_verts:
                    ori_x, ori_y = self._x_at_down[i], self._y_at_down[i]   
                    self.x[i] = ori_x + dx
                    self.y[i] = ori_y + dy
        ...
        if self.completed:
            ...
            plast = self._to_window((self.x[-1], self.y[-1]))
            pfirst = self._to_window((self.x[0], self.y[0]))
            dist = np.linalg.norm(pfirst - plast)
    ...
    def highlight(self):
        if len(self._active_verts) == 0:
            ...
            self.edge_hl.set_data(array([(self.x[i], self.y[i]) for i in self._active_verts]).T)
            self.edge_hl.set_visible(True)
        
  
    def _distance_to_edge(self, p0, p1, xy):
        #breakpoint()
        p0 = np.array(p0)
        p1 = np.array(p1)
        xy = np.array(xy)
        v = p1-p0
        l = np.linalg.norm(v) 
        q = v.dot(xy-p0)/l  # Distance along the edge
        q = np.clip(q, 0, l) # Make sure we are on the line segment
        p = p0 + v*q/l # Closest point onthe line segment
    ...
    def collapse_edge(self, i=None, xy=None):
        ...
        i %= len(self) # Possibly wrap around sinec we may have been the last edge
        self.x[i] = xy[0]
        self.y[i] = xy[1]
    ...
    def update(self):
        ...
        # Update the hightlight
        self.highlight()
        
        if self.onupdate:
            self.onupdate(self.x, self.y)
    ...
    def edge_normal(self, i, length=1):
        ...
        l = np.hypot(dx, dy)
        return np.array((-length*dy/l, length*dx/l))
