...
class PolygonSelector(object):
    ...
    - <kbd>wheel</kbd> to zoom in and out around the mouse cursor
    ...
    def add_polygon(self,
                    xy: Union[Polygon, np.ndarray],
                    selectable=True,
                    **kwargs) -> Polygon:
        return self.insert_polygon(len(self.polygons), xy, selectable, **kwargs)

    def delete_polygon(self, index: int) -> Polygon:
    ...
    def refresh_polygon(self, index: int):
    ...
    def select_at(self, xy):
        if not self.select_enabled:
            return
        self.set_active_index(self.find_polygon(xy))
    ...
    def select_next(self, selectable=True, direction=1):
        ...
        if self.active_index == -1 and direction == -1:
            i = len(self.polygons) - 1
        else:
            i = (self.active_index + direction)

        if selectable:
            found = False
            while -1 < i < len(self.polygons):
